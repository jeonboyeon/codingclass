...
!(function (a, b) {
    ...
    function g(a) {
        ...
        return q / l >= 0.75 && (k = Math.floor(0.75 * k * (l / q))), (b.font = 'bold ' + k * e + 'px ' + n), b.fillText(p, l / 2, m / 2, l), o.toDataURL('image/png');
    }
    function h(a, b, c, d) {
        var f = c.dimensions,
            h = c.theme,
        ...
        (h = i ? e(h, { text: i }) : h),
            (h = c.font ? e(h, { font: c.font }) : h),
            b.setAttribute('data-src', d),
            (c.theme = h),
            (b.holder_data = c),
            'image' == a
                ? (b.setAttribute('alt', i ? i : h.text ? h.text + ' [' + j + ']' : j),
                  (n || !c.auto) && ((b.style.width = f.width + 'px'), (b.style.height = f.height + 'px')),
                  n ? (b.style.backgroundColor = h.background) : (b.setAttribute('src', g({ ctx: s, dimensions: f, template: h, ratio: t, holder: c })), c.textmode && 'exact' == c.textmode && (r.push(b), k(b))))
                : 'background' == a
                ? n || ((b.style.backgroundImage = 'url(' + g({ ctx: s, dimensions: f, template: h, ratio: t, holder: c }) + ')'), (b.style.backgroundSize = f.width + 'px ' + f.height + 'px'))
                : 'fluid' == a &&
                  (b.setAttribute('alt', i ? i : h.text ? h.text + ' [' + j + ']' : j),
                  (b.style.height = '%' == f.height.slice(-1) ? f.height : f.height + 'px'),
                  (b.style.width = '%' == f.width.slice(-1) ? f.width : f.width + 'px'),
                  ('inline' == b.style.display || '' === b.style.display || 'none' == b.style.display) && (b.style.display = 'block'),
                  n ? (b.style.backgroundColor = h.background) : (r.push(b), k(b)));
    }
    function i(a, b) {
        ...
        if (!c.height && !c.width) {
            ...
            return (
                a.setAttribute('data-holder-invisible', !0),
                setTimeout(function () {
                    b.call(this, a);
                }, 1),
                null
            ...
            if (b.hasOwnProperty(c)) {
                ...
                if (d.holder_data) {
                    var e = d.holder_data,
                        f = i(d, k);
                    f &&
                        (e.fluid && d.setAttribute('src', g({ ctx: s, dimensions: f, template: e.theme, ratio: t, holder: e })), e.textmode && 'exact' == e.textmode && ((e.exact_dimensions = f), d.setAttribute('src', g({ ctx: s, dimensions: e.dimensions, template: e.theme, ratio: t, holder: e }))));
                }
            }
    }
    function l(b, c) {
        var d = { theme: e(u.themes.gray, {}) },
            f = !1;
        for (sl = b.length, j = 0; sl > j; j++) {
            var g = b[j];
            a.flags.dimensions.match(g)
                ? ((f = !0), (d.dimensions = a.flags.dimensions.output(g)))
                : a.flags.fluid.match(g)
                ? ((f = !0), (d.dimensions = a.flags.fluid.output(g)), (d.fluid = !0))
                : a.flags.textmode.match(g)
                ? (d.textmode = a.flags.textmode.output(g))
                : a.flags.colors.match(g)
                ? (d.theme = a.flags.colors.output(g))
                : c.themes[g]
                ? c.themes.hasOwnProperty(g) && (d.theme = e(c.themes[g], {}))
                : a.flags.font.match(g)
                ? (d.font = a.flags.font.output(g))
                : a.flags.auto.match(g)
                ? (d.auto = !0)
                : a.flags.text.match(g) && (d.text = a.flags.text.output(g));
        }
        return f ? d : !1;
    }
    var m = !1,
    ...
    for (var v in a.flags)
        a.flags.hasOwnProperty(v) &&
            (a.flags[v].match = function (a) {
    ...
    (a.add_theme = function (b, c) {
        ...
        (a.run = function (b) {
            ...
            var c = e(u, b),
                ...
                i = [];
            for (
                'string' == typeof c.images ? (g = d(c.images)) : window.NodeList && c.images instanceof window.NodeList ? (g = c.images) : window.Node && c.images instanceof window.Node && (g = [c.images]),
                    'string' == typeof c.bgnodes ? (i = d(c.bgnodes)) : window.NodeList && c.elements instanceof window.NodeList ? (i = c.bgnodes) : window.Node && c.bgnodes instanceof window.Node && (i = [c.bgnodes]),
                    o = 0,
                    n = g.length;
                n > o;
                o++
    ...
    !(function (a) {
        a(function () {
            ...
            d.scrollspy({ target: '.bs-sidebar' }),
                ...
                a('.bs-docs-navbar').tooltip({ selector: 'a[data-toggle=tooltip]', container: '.bs-docs-navbar .nav' }),
